...
export function CreditModal({
    ...
    const activeCompanies = companies.filter(co => co.isActive)
    const filteredCompanies = activeCompanies.filter(co =>
        !companySearch || normalizeStr(co.name).includes(normalizeStr(companySearch))
    )
    const selectedCompany = activeCompanies.find(co => co.id === selectedCompanyId)
    ...
    const modalTitle = mode === 'split' ? 'Crédito dividido' : mode === 'company' ? 'Crédito a empresa' : 'Venta a crédito'
    const modalDesc = mode === 'split' ? 'Divide productos entre varias cuentas' : mode === 'company' ? 'Selecciona la empresa' : 'Selecciona el cliente'
            ...
            <div className="flex gap-1 p-1 rounded-xl bg-[#101520] border border-[#192030] mb-4">
                ...
                {activeCompanies.length > 0 && (
                    <button
                        ...
                        className={cn(
                            'flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[12px] font-semibold transition-all cursor-pointer',
            ...
            {mode === 'company' && (
                <div className="space-y-4">
                    ...
                    <div className="space-y-1.5 max-h-52 overflow-y-auto">
                        {filteredCompanies.map(co => {
                            ...
                            return (
                                <button
                                    key={co.id}
                                    onClick={() => { useKeyboardStore.getState().close(); setSelectedCompanyId(co.id) }}
                                    className={cn(
                                        'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all cursor-pointer text-left',
                                        isSelected
                                ...
                                >
                                    <Building2 size={16} className={isSelected ? 'text-orange-400' : 'text-[#3D506A]'} />
                                    <div className="flex-1 min-w-0">
                                        <p className={cn('text-[13px] font-medium truncate', isSelected ? 'text-[#E4ECF7]' : 'text-[#7A8FAA]')}>{co.name}</p>
                                        {co.billingEmail && <p className="text-[11px] text-[#3D506A] truncate">{co.billingEmail}</p>}
                                    </div>
                        ...
                        {filteredCompanies.length === 0 && (
                            <p className="text-center text-[12px] text-[#3D506A] py-6">Sin empresas encontradas</p>
                        )}
                    </div>
                    ...
                    <Button
                        ...
                        loading={isPending}
                        onClick={() => selectedCompanyId && onConfirmCompany(selectedCompanyId)}
                    >
                        {selectedCompany ? `Cargar a ${selectedCompany.name} — ${formatCurrency(total)}` : 'Selecciona una empresa'}
                    </Button>
                </div>
            )}
